...
class PageLinesBoxes extends PageLinesSection {
   ...
   function section_template() {    
	...
	function draw_boxes($b, $perline = 3, $class = ""){ 
		...
		$count = $perline;
?>
		<div class="<?php echo $class;?> dcol_container_<?php echo $perline;?> fix">
...
?>
			<div id="<?php echo 'fbox_'.$bpost->ID;?>" class="dcol_<?php echo $perline;?> dcol">
			...
			return array(
				'box_settings' => array(
						'box_col_number' => array(
								...
								'docslink'		=> 'http://anonym.to/?http://www.pagelines.com/docs/boxes-soapboxes', 
...
function pagelines_default_boxes($post_type){
	...
	foreach($default_posts as $dpost){
		...
		$object_terms = array('default-boxes');
		
		wp_set_object_terms($newPostID, 'default-boxes', 'box-sets');
		
		// Add other default sets, if applicable.
		if(isset($dpost['set'])) wp_set_object_terms($newPostID, $dpost['set'], 'box-sets', true);
